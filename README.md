...
- **A file still looks out of sync after a sync has run** (a note that won't update, or the two sides disagree, and repeated syncs don't fix it): Open **Settings → Air Sync → Advanced** and click **Rescan**. It re-checks your whole vault against your cloud storage from scratch — rather than only what changed since the last sync — and clears stale local rename constraints for the selected cloud folder. Existing sync records and merge history stay intact, so only genuine differences are transferred.
