...
After scope projection, `prefer_local` alone completes SHA-256 facts for included, same-path bilateral edits that have a non-empty committed baseline. Admission uses those facts to distinguish a proven local-win conflict from one that must preserve both versions. Other strategies perform no reads for this proof, excluded paths are not read, and missing proof—including a cold start with no baseline—routes to preservation rather than local overwrite.
...
`sync-records` is keyed by `remoteIdentityKey` — the provider object's identity, not its address — with a **unique index over `path`**; `sync-content` (the merge-base store) is keyed the same way. So a record's key never moves when the file does, and every production commit goes through a compare-and-swap that compares its captured expectations against one transaction image before it writes anything.

`commitAction()` in `state-committer.ts` persists state per successfully-executed action:

- `push` / `pull` / `match` / `conflict`: publish the terminal record with whole-record `stateStore.compareAndPut(source, record, destination)`. Two captured expectations are compared inside the one transaction: `source`, the row this publication continues (looked up by the terminal's own `remoteIdentityKey`), and `destination`, whatever the `path` index shows occupying the claimed address. Neither is defaulted from the other. A baseline-free new action passes both as `undefined`, which requires the address to be vacant. A stale expectation fails the action instead of overwriting a newer winner, and because every comparison precedes every write, a failed publication writes and deletes nothing. If `enableThreeWayMerge` is on and the file is merge-eligible (`isMergeEligible`: byte size <= 1 MiB (`MAX_MERGE_SIZE = 1024*1024`) and the file extension is in the fixed `TEXT_EXTENSIONS` allowlist — .md/.txt/.json/.canvas/.css/.js/.ts/... — not a content sniff), the file content is stored as a future 3-way merge base with `stateStore.compareAndPutContent(record, content)`, which writes only while the terminal record is still exactly the row that was just published. The content store compresses each entry (raw deflate via `store/content-codec.ts`, prefixed with a 1-byte format header; tiny/incompressible entries fall back to stored-raw so they never grow). Compression is transparent: the content writers encode and `getContent` decodes, so callers always handle plain bytes. A relocation never copies or re-encodes stored bytes — the content row shares the record's identity key, so it simply stays put; a publication drops it only when it invalidates the base (no continued row, no hash, or a changed hash, local size or remote identity).
- `rename_remote` / `rename_local`: for a folder rename (`isFolder` with `descendantRecords`), `stateStore.compareAndRewritePaths(relocations)` publishes every child in one IndexedDB transaction — it compares each child's captured source row and captured destination occupant against a single transaction image, then updates each identity-keyed row in place. The whole set is refused (nothing published, `false` returned) when it is non-injective in source or terminal addresses, or when one item's terminal address is a *different* item's source address (a cyclic set is one such arrangement); self-overlap, where an unmoved item's terminal address is its own source address, is admitted. A single-file rename takes the ordinary `compareAndPut` route above: the row's key is its remote identity, so the rename is one `put` of that same row with a new `path`, the row it continues is never deleted, and the identically keyed merge-base content stays in place. (A record for a *different* identity found holding the claimed address is a replacement: it is compared first and then deleted in the same transaction.)
- `delete_local` / `delete_remote` / `cleanup`: `stateStore.compareAndDelete(path, destination)` removes the captured record *and* its merge-base content, after proving in the same transaction that the captured record is still both that row and the occupant of `path`. With no captured record it verifies the address is vacant, deletes nothing and returns true.

`put()`, `putContent()` and `delete()` remain on the store as uncompared setup/test routes and have **no** production caller; `clear()` is reached only when the backend identity changes or the target is disconnected/reset. Execution publishes exclusively through the four compare-and-swap routes above.
