...
For an opened path with no `SyncRecord`, the priority owner returns `deferred_to_vault_debounce` without requesting a normal cycle itself. `pullSingle()` propagates that result to `SyncScheduler`, and the scheduler calls its existing `debouncedSync()`. The scheduler therefore remains the only owner of vault timing: an existing create/rename/modify timer is reset, and a timer that already fired before an incomplete cycle is re-armed. This also does not depend on whether `file-open` or `create` is observed first. No dirty mark is consumed.

Once a baseline exists, unavailable priority capability, an active-batch deferral, a record without remote identity, and actual priority failures retain their immediate normal-lifecycle requests. Manual, startup, and foreground triggers keep their existing timing.
