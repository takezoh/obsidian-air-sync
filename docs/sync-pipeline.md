...
The **Rescan vault** action (settings → Advanced) is a target-scoped manual recovery boundary. Under the sync mutex it discards the committed checkpoint via the live FS (`remoteFs.checkpoint.resetCheckpoint()`), clears only `RenameDebt` for the current backend/root namespace, removes only matching in-memory local rename evidence, and forces a cold reconcile. Baselines, merge bases, other target namespaces, and pending remote evidence remain intact. Checkpoint reset happens before debt deletion, so a reset failure retains the constraint. See [ADR 0009](adr/0009-proved-local-folder-fallback-and-targeted-rescan.md).
...
- **Folder renames** (`coalesceLocalFolderRenames`): Consumes the derived folder view and coalesces all mapped descendant actions into one `rename_remote` with `isFolder: true`. Every descendant must pass hash verification. If a local folder mapping is incomplete, Admission may use the original file actions only when each consequence is independently proved, scope is wholly included, and no remote/ambiguous action is present; otherwise the component still defers ([ADR 0009](adr/0009-proved-local-folder-fallback-and-targeted-rescan.md)).
