...
written and the remote terminal proves those bytes. The immutable captured local entity
is the historical local half of that transfer's `SyncRecord`; a later local rename,
replacement, or same-path edit is ordinary tracker input and converges from that
baseline. [ADR 20260914](adr-20260914-publish-captured-push-revision.md) supersedes the
former disappearance-only restriction. This exception remains local-push-only: an
unproved remote terminal or a disappearing/changed remote source during pull stays
non-clean.
