...
| shared-provider-contract | T1 contract | `npm test -- src/fs/ifilesystem-contract.test.ts src/fs/caching/remote-fs-contract.test.ts` | Mandatory authority, identity, absence/error, order-independent rename, and snapshot cases pass for each faithful fake. |
