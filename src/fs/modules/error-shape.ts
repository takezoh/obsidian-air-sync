...
import { AuthError, getErrorInfo } from "../errors";
 ...
 *
 * An `auth` shape additionally becomes a real {@link AuthError}: the cycle-abort
 * decision in `plan-executor` is `err instanceof AuthError`, so an auth failure
 * must keep that identity or it would be recorded as an ordinary action failure
 * instead of aborting the cycle and prompting reconnect. The structural shape is
 * still attached so `isBackendErrorShape`/`classifyBackendError` keep working.
 */
