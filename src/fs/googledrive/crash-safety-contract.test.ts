...
// mock backend), via a minimal DriveClient stub over an in-memory remote — a baseline
// of DriveFiles plus an append-only changes.list log. This proves the A1 lift kept
// Drive's ADR 0001 path-1 behaviour intact through the new seams, and answers
...
function makeDriveHarness(): CachingRemoteFsHarness<DriveFile> {
	const baseline = new Map<string, DriveFile>();
	const events: DriveChange[] = [];
	let idSeq = 0;
	...
	return {
		makeStore: (id) => new MetadataStore<DriveFile>(id, { dbNamePrefix: "air-sync-drive-contract", version: 1 }),
...
runCachingRemoteFsContract("GoogleDriveFs", makeDriveHarness);
