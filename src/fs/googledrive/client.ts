...
export class GoogleDriveClient {
	...
	private async request(
		...
		try {
			...
			if (status === 401 && !retried) {
				this.logger?.info("Google Drive API returned 401, refreshing token and retrying", { operation });
	...
	/**
	 * Return every same-parent/name candidate. Unlike {@link findChildByName}, this is
	 * suitable for synchronization authority: all pages are drained so duplicate Drive
	 * names become an explicit conflict instead of an arbitrary first match.
	 */
	...
	async getFile(fileId: string): Promise<GoogleDriveFile> {
		...
		const response = await this.request("getFile", {
			url: `${GOOGLE_DRIVE_API}/files/${fileId}?${params.toString()}`,
		});
		const result: unknown = response.json;
		assertGoogleDriveFile(result);
		return result;
	}
	...
	async createFolder(name: string, parentId: string): Promise<GoogleDriveFile> {
		const response = await this.request("createFolder", {
			url: `${GOOGLE_DRIVE_API}/files?fields=${FILE_FIELDS}`,
			method: "POST",
			headers: {
	...
	async getChangesStartToken(): Promise<string> {
		const response = await this.request("getChangesStartToken", {
			url: `${GOOGLE_DRIVE_API}/changes/startPageToken`,
		});
		const result: unknown = response.json;
	...
	async listChanges(
		...
		const response = await this.request("listChanges", {
			url: `${GOOGLE_DRIVE_API}/changes?${params.toString()}`,
		});
		const result: unknown = response.json;
		assertGoogleDriveChangeList(result);
		return result;
	}
}
