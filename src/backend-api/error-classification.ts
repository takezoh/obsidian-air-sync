import { isBackendErrorShape } from "./errors";
...
/**
 * Whether an error is an authentication failure that must abort the cycle.
 *
 * The check is structural first: a backend module can be a separately bundled
 * artifact whose `AuthError` class identity differs from core's, so a shape
 * carrying `kind: "auth"` (the Backend Module API contract) is authoritative.
 * Core's own `AuthError` is still honored for errors raised inside the core
 * bundle.
 */
export function isAuthFailure(err: unknown): boolean {
	if (isBackendErrorShape(err) && err.kind === "auth") return true;
...
/**
 * Normalize any thrown value to an `Error`, preserving a structural
 * `BackendErrorShape.message`. The public contract lets a module throw a plain
 * shape object, but the engine's throw/report convention is `Error`; without
 * this, `String(err)` would collapse the safe diagnostic message to
 * `"[object Object]"`.
 */
export function toError(err: unknown): Error {
	if (err instanceof Error) return err;
	return Object.assign(new Error(messageOf(err)), err as object);
}

/**
 * The safe diagnostic message of any thrown value, including a plain structural
 * `BackendErrorShape`. Prefer this over `err instanceof Error ? err.message :
 * String(err)`, which collapses a plain shape to `"[object Object]"`.
 */
...
/**
 * Normalize an auth failure to an `AuthError` before it is thrown or reported.
 * A class-identity `AuthError` is returned unchanged.
 */
export function toAuthError(err: unknown): AuthError {
	if (err instanceof AuthError) return err;
	return Object.assign(new AuthError(messageOf(err), 401), err as object);
}
...
export function classifyHttpError(err: unknown): ErrorClassification {
	if (isAuthFailure(err)) return { kind: "auth" };
