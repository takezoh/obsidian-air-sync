...
/**
 * Cooperative safe point between indivisible normal actions. Priority work stops
 * new normal admissions, waits only for permits already in use, then drains in
 * FIFO path order. The coordinator owns no filesystem or sync state.
 */
export class PriorityCoordinator {
	...
	/** Run checkpoint/final state mutation at the same safe point as priority work. */
