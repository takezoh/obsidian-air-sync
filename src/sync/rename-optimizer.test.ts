...
describe("refinePlan", () => {
		...
		const p = plan([
			{
				path: "local-old.md",
				action: "delete_remote",
				remote: entity("local-old.md", "h1"),
				baseline: baseline("local-old.md", "h1"),
			},
			{
				path: "local-new.md",
				action: "push",
				local: entity("local-new.md", "h1"),
			},
			{
				path: "remote-old.md",
				action: "delete_local",
				local: entity("remote-old.md", "h2"),
				baseline: baseline("remote-old.md", "h2"),
			},
			{
				path: "remote-new.md",
				action: "pull",
				remote: entity("remote-new.md", "h2"),
			},
		]);
		...
		const p = plan([
			{
				path: "x-old.md",
				action: "delete_remote",
				remote: entity("x-old.md", "h1"),
				baseline: baseline("x-old.md", "h1"),
			},
			{
				path: "x-new.md",
				action: "push",
				local: entity("x-new.md", "h1"),
			},
			{
				path: "y-old.md",
				action: "delete_local",
				local: entity("y-old.md", "h2"),
				baseline: baseline("y-old.md", "h2"),
			},
			{
				path: "y-new.md",
				action: "pull",
				remote: entity("y-new.md", "h2"),
			},
			{
				path: "unrelated.md",
				action: "push",
				local: entity("unrelated.md", "h3"),
			},
		]);
	...
	it("coalesces local folder rename with remaining file renames", () => {
		const p = plan([
			...
			{ path: "B/f1.md", action: "push", local: entity("B/f1.md", "h1") },
			{
				path: "other-old.md",
				action: "delete_remote",
				remote: entity("other-old.md", "h2"),
				baseline: baseline("other-old.md", "h2"),
			},
			{
				path: "other-new.md",
				action: "push",
				local: entity("other-new.md", "h2"),
			},
		]);
		...
		const folder = result.actions.find(
			(a) => "isFolder" in a && a.isFolder,
		);
	...
	it("returns a new plan object without mutating the input when actions change", () => {
		const p = plan([
			{
				path: "old.md",
				action: "delete_remote",
		...
		expect(p.actions.map((a) => a.action)).toEqual([
			"delete_remote",
			"push",
		]);
	});
