...
describe("PriorityCoordinator", () => {
	it("admits only the already-started normal action before a queued priority attempt", async () => {
	...
	it("drains priority before finalization and normal admission resumes after seal", async () => {
		const coordinator = new PriorityCoordinator();
		const normal = await coordinator.acquireNormalPermit();
		const order: string[] = [];
		...
		const finalization = coordinator.finalize(() => {
			order.push("finalize");
			return Promise.resolve();
		});
		normal.release();
