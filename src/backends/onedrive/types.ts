import type { RemoteChecksum } from "../../backend-api";
...
export function toRemoteChecksum(item: OneDriveItem): RemoteChecksum | undefined {
	...
	if (hashes?.quickXorHash) return { algorithm: "quickxor", value: hashes.quickXorHash };
	if (hashes?.sha256Hash) return { algorithm: "sha256", value: hashes.sha256Hash.toLowerCase() };
	if (hashes?.sha1Hash) return { algorithm: "sha1", value: hashes.sha1Hash.toLowerCase() };
	return undefined;
}
