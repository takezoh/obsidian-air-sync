...
3. **Delta-first** -- Only process files that changed. O(n) full scans are allowed on cold start, scope change, manual rescan, and unresolved local rename debt. Ordinary same-session incomplete work replays its uncommitted delta without a full scan.
...
5. **Crash-safe by construction** -- State is committed only *after* success: existing per-file baselines use CAS, and the remote delta checkpoint advances only after exact authorized component/member completion with no failure/deferral or unresolved remote identity edge. An interrupted sync converges by same-session delta replay or restart replay; unresolved local rename edges additionally survive as bounded namespace debt.
...
| `sync/` | The sync pipeline and its orchestration: change tracking and evidence-complete detection (hot/warm/cold), scope projection, the decision engine, rename optimization, fail-closed direction-free component admission, late-bound execution (3-phase lane/tier scheduling), per-action CAS state commit, exact component/member checkpoint finalization, conflict resolution and 3-way merge, the orchestrator (mutex/retry/status), the scheduler (vault events + triggers), the IndexedDB `SyncStateStore`, error classification, and the conflict-history audit writer. |
...
└────────────┬──────────────────────┬──────────────────┘
     ...
     ┌───────▼──────────────────────▼──────┐
     │         SyncOrchestrator            │
     │ mutex, retry loop, priority safe    │
     │ point, action CAS, finalization     │
     └───────────────┬────────────────────┘
                     │
     ┌───────────────▼────────────────────┐
     │            Pipeline                │
...
interface IFileSystem {
  ...
  priority?: PriorityObservationCapability; // detached identity + independent path-occupant observation
...
- `priority` is an optional remote-only capability. It independently observes the stable
  identity stored in the file's `SyncRecord` and the current path occupants (absent, one
  matching/current object, multiple conflicting objects, or unverifiable), proving a complete
  request-local path to the configured root. It never reads or mutates the shared metadata
  cache, root-path anchor, delta cursor, or checkpoint.
  Generic `stat()` remains cache-backed.
