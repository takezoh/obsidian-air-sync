...
5. **Crash-safe by construction** -- State is committed only *after* success: per-file baselines after each admitted action, and the remote delta checkpoint only when no failure or retryable unknown remains. An interrupted sync is freshly reclassified on the next invocation, using a same-session COLD reconcile when required. Legacy local rename rows retain candidate endpoints for COLD acquisition but never authorize an effect.
...
The remote delta cursor is crash-safe at the **filesystem** layer, not the provider (it moved there with ADR 0001): `IFileSystem.checkpoint` commits the cursor plus its derived cache to the backend's own IndexedDB store atomically, and only after a cycle has no failed work or retryable unknown identity component. A missing checkpoint (first sync, cleared state, manual rescan) forces COLD. After a non-clean cycle with an older checkpoint still committed, same-session recovery forces COLD despite the advanced live cursor; restart reloads that older committed cursor and reacquires current facts. Scope changes and persisted local rename candidate rows also force COLD. A v6 `RenameDebt` row is unchanged legacy endpoint evidence: it may widen COLD acquisition, cannot authorize an effect, and its exact key is deleted only after a fresh proven consequence and successful checkpoint commit. `readBackendState()` persists only non-secret provider/auth state (e.g. token expiry) to `settings.backendData`; it no longer carries the cursor, so it takes no FS argument.
