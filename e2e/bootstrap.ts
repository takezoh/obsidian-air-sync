 ...
 * the shipped auth code paths (`GoogleAuthDirect`, `DropboxAuth`, and the shared
 * `buildDropboxAuthorizeUrl`) so the e2e exercises the production token exchange.
 *
...
async function main(): Promise<void> {
	...
	if (which === "google") await bootstrapGoogle();
		...
		stdout.write("Usage: npm run e2e:bootstrap -- <google|dropbox>\n");
